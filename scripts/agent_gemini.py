#!/usr/bin/env python3
"""
ROS2 Drone Agent Node — Gemini 2.0 Flash
Subscribes to /drone/pose (String) and /camera/image_raw (Image),
calls Google Gemini 2.0 Flash with vision, and publishes commands to /motion_command (String).
...
# ═══════════════════════════════════════════════════════════════
#  API Key — reads from gemini_api_key.txt next to this script
# ═══════════════════════════════════════════════════════════════
_KEY_FILE = Path(__file__).resolve().parent / "gemini_api_key.txt"
...
def encode_image_bytes(cv_img) -> bytes:
    """BGR OpenCV image → JPEG bytes."""
...
class DroneAgent(Node):
    ...
    def __init__(self):
        super().__init__("drone_agent_gemini")
        ...
        self.get_logger().info(
            f"DroneAgent (Gemini) started. Goal → X={GOAL_X}, Y={GOAL_Y}, Z={GOAL_Z} | "
    ...
    def _run_agent(self, pose: Pose, image, dist: float):
        ...
        try:
            ...
            # Encode image as inline data for Gemini
