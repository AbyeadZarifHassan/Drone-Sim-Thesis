...
# ─────────────────────────────────────────────
OPENAI_MODEL   = "gpt-4o"
ARRIVAL_THRESH = 3      # metres – agent stops issuing commands inside this radius

# ── Handoff ──────────────────────────────────────────────────────────────────
# When the drone is within this distance of the goal, the LLM agent is
# deactivated and "DO" is published to /path_plan_activate so the Unity
# PathPlanner takes over for the final precise approach.
# Must be larger than ARRIVAL_THRESH.
PATHPLANNER_HANDOFF_RADIUS: float = 30.0   # metres  ← adjust as needed
...
class DroneAgent(Node):
    ...
    def __init__(self):
        ...
        # ── handoff state ────────────────────────────────────────────────────
        # Once True, the LLM agent is permanently silent and PathPlanner owns
        # the drone. The flag is set exactly once so "DO" is published only once.
    ...
    def _trigger_pathplanner_handoff(self, dist: float):
        """
        Deactivate the LLM agent and hand control to the Unity PathPlanner
        by publishing "DO" to /path_plan_activate (exactly once).
        """
    ...
    def _decision_loop(self):
        ...
        # ── If PathPlanner has taken over, agent is permanently silent ────────
        with self._lock:
        ...
        # ── Check: has the drone entered the handoff zone? ────────────────────
        if dist <= PATHPLANNER_HANDOFF_RADIUS:
            self._trigger_pathplanner_handoff(dist)
            return

        # ── Normal arrival guard (should rarely fire given handoff above) ─────
    ...
    def _run_agent(self, pose: Pose, image, dist: float):
        try:
            ...
            # ── Re-check: handoff may have been triggered while LLM was running ──
            with self._lock:
                if self._pathplanner_active:
                    self.get_logger().info(
